...
| CORE-187 | Реализовать reviewPromptAssemblerService | DONE | Реализовано | Реализация: функциональность сборки секций перенесена в `application/shared/prompt-resolution.ts` и `application/shared/prompt-override-variables.ts`. Отдельный service удалён как мёртвый (см. CORE-205). Готово, если: сборка промптов и override-приоритеты подтверждены тестами shared-утилит и поведение stages соответствует ожиданиям; DoD: `cd packages/core && bun run lint && bun run typecheck && bun test`. |
...
| CORE-205 | Реализовать remove dead ReviewPromptAssemblerService | DONE | Реализовано | Реализация: Удалён service + очищены экспорты из `domain/index.ts` и корневого `src/index.ts`. Готово, если: для CORE-205 полностью выполнены пункты блока «Реализация», поведение подтверждено unit/integration тестами (happy-path, negative, edge-case), контракты DTO/ports совместимы, регрессии в смежных use case отсутствуют; DoD: `cd packages/core && bun run lint && bun run typecheck && bun test`. |
...
| CORE-219 | Реализовать importSystemSettingsUseCase | DONE | Реализовано | Реализация: `application/use-cases/common/import-system-settings.use-case.ts` + порт `ISystemSettingsWriter`. Config DTO `system-setting-config.dto.ts` с parseSystemSettingConfigList. Idempotent по key (skip existing). 5 тестов. Готово, если: для CORE-219 полностью выполнены пункты блока «Реализация», поведение подтверждено unit/integration тестами (happy-path, negative, edge-case), контракты DTO/ports совместимы, регрессии в смежных use case отсутствуют; DoD: `cd packages/core && bun run lint && bun run typecheck && bun test`. |
