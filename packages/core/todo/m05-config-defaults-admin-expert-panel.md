...
| ID | Задача | Статус | Результат | Acceptance Criteria |
|----------|--------------------|--------|-----------|---------------------|
| CORE-183 | Реализовать importDefaultRuleCategoriesUseCase | TODO | Не начато | Реализация: `application/use-cases/rules/import-default-rule-categories.use-case.ts`. IUseCase<IConfigRuleCategoryItem[], IImportResult>. RuleCategoryFactory.create() → IRuleCategoryRepository.save(). Idempotent: skip если slug существует (findBySlug). Источник — settings-service `/configs/categories`. Готово, если: для CORE-183 полностью выполнены пункты блока «Реализация», поведение подтверждено unit/integration тестами (happy-path, negative, edge-case), контракты DTO/ports совместимы, регрессии в смежных use case отсутствуют; DoD: `cd packages/core && bun run lint && bun run typecheck && bun test`. |
