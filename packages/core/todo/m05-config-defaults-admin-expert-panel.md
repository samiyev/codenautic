...
| CORE-188 | Реализовать iPromptOverrides в IReviewConfig | DONE | Реализовано | Реализация: расширен `IReviewConfigDTO` и exports, добавлен `v2PromptOverrides?: { categories?: { descriptions?: { bug?: string, performance?: string, security?: string } }, severity?: { flags?: { critical?: string, high?: string, medium?: string, low?: string } }, generation?: { main?: string } }`. ConfigurationValidator принимает и валидирует вложенные секции, сохраняет совместимость с `promptOverrides`. Тесты покрывают валидный и невалидный ввод. Готово, если: для CORE-188 полностью выполнены пункты блока «Реализация», поведение подтверждено unit/integration тестами (happy-path, negative, edge-case), контракты DTO/ports совместимы, регрессии в смежных use case отсутствуют; DoD: `cd packages/core && bun run lint && bun run typecheck && bun test`. |
