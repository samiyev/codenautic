...
import { useCustomRules, type IUseCustomRulesResult } from "@/lib/hooks/queries/use-custom-rules"
import {
    CUSTOM_RULE_SCOPE,
...
function CustomRulesState(hook: IUseCustomRulesResult): ReactElement {
    ...
    return (
        <div>
            <p data-testid="custom-rules-count">{hook.customRulesQuery.data?.rules.length ?? 0}</p>
...
describe("useCustomRules", (): void => {
    ...
    it("создаёт правило с оптимистичным обновлением", async (): Promise<void> => {
        ...
        renderWithProviders(<CustomRulesProbe />)
        expect(await screen.findByTestId("custom-rules-titles")).toHaveTextContent("Current rule")
